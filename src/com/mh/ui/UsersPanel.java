...
public class UsersPanel extends JPanel{
	...
	private JTextField husbandName;
	...
	private List<String>searchHistory = new ArrayList<String>();//for enabling searchhistory(backbutton) 
	private JButton backSearch;
	private JButton forwardSearch;
	...
	public UsersPanel(Main main) {
		...
		gridBagLayout.columnWidths = new int[]{10, 100, 100, 100 , 20,20, 20, 0, 10};
		...
		backSearch = new JButton(ImageUtil.getIcon(HMConstants.BACK_ICON));
		backSearch.setContentAreaFilled(false);
		backSearch.setBorderPainted( false );
		backSearch.setMinimumSize(new Dimension(30, 30));
		backSearch.setPreferredSize(new Dimension(40, 40));
		backSearch.setMaximumSize(new Dimension(50,Short.MAX_VALUE));
		backSearch.setToolTipText("<html>Back</html>");
		
		GridBagConstraints gbc_backSearch = new GridBagConstraints();
		gbc_backSearch.insets = new Insets(0, 0, 5, 5);
		gbc_backSearch.gridx = 1;
		gbc_backSearch.gridy = 0;
		gbc_backSearch.anchor = GridBagConstraints.LINE_START;
		add(backSearch, gbc_backSearch);
		backSearch.setVisible(false);
		backSearch.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				searchHistory.remove(historyIndex--);
				searchText.setText(searchHistory.get(historyIndex));
				//searchAction(searchHistory.get(historyIndex));//Need  to change
				clearSearchHistory();
			}
		});
		...
		add(dob, gbc_comboBox1);
		
		husbandName= new RoundJTextField(10);		
		husbandName.setFont(new Font("Arial",Font.BOLD,15));
		/*Border roundedBorder = new LineBorder(new Color(210,210,0), 14, true);*/
		...
		gbc_comboBox3.gridx = 3;
		gbc_comboBox3.gridy = 2;
		add(husbandName, gbc_comboBox3);
		...
		btnSearch.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				if(Main.session.isValid()){
					...
					String dateOfBirth = dob.getDateFormatString();
					String husband = husbandName.getText();
					
					if(target!=null &&searchHistory.size()==0 ){
						searchHistory.add(target);
						historyIndex=0;
					}else if(target!=null && !target.equals(searchHistory.get(historyIndex))){
						searchHistory.add(target);
						historyIndex++;
					}
					searchAction(target,dateOfBirth,husband);
				}
			}

		});
		/*refreshButton = new JButton(ImageUtil.getIcon(HMConstants.HOME_ICON));//home button
		refreshButton.setContentAreaFilled(false);
		refreshButton.setBorderPainted( false );
		refreshButton.setMinimumSize(new Dimension(30, 30));
		refreshButton.setPreferredSize(new Dimension(30, 30));
		refreshButton.setMaximumSize(new Dimension(30,Short.MAX_VALUE));
		refreshButton.setToolTipText("<html>Home</html>");
		GridBagConstraints gbc_btnRefresh = Utils.getConStraints(5, 2, null);
		...
		/*refreshButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				if(Main.session.isValid()){
					//setTable(HMConstants.EDIT);//uncomment for full user list while home(refresh)
					clearTable();
					clearSearchHistory();
				}
			}
	...
	//search result  end	
		...
		dm.addColumn("Edit");
//		dm.addColumn("Reg No");
		dm.addColumn("Name of Patient");
		dm.addColumn("DOB(yyyy-mm-dd)");
	//	dm.addColumn("Sex");
	//	dm.addColumn("Center");
	//	dm.addColumn("Care of");
		dm.addColumn("Husband");
		dm.addColumn("Delete");
	...
	/*	TableRowSorter<DefaultTableModel> sorter = new TableRowSorter<DefaultTableModel>(dm);
	   ...
	   table.getColumn("Name of Patient").setCellRenderer(new CellRenderer(b2,f2));
	   table.getColumn("DOB(yyyy-mm-dd)").setCellRenderer(new CellRenderer(b1,f1));
	   ...
	   table.getColumn("Husband").setCellRenderer(new CellRenderer(b2,f2));
	    ...
	    table.getColumn("Delete").setMinWidth(70);
		...
		protected JButton button;
		  ...
		  public Component getTableCellEditorComponent(JTable table, Object value,
		   ...
		   // button.setText(label);
		    isPushed = true;
		    userId = (Long) table.getModel().getValueAt(row, 5);
		    user = (User) table.getValueAt(row, 6);
		    return button;
		  }
	...
	private void searchAction(String target,String dateOfBirth,String husband) {
		if((target!=null && !target.trim().equals(""))||(dateOfBirth!=null && !dateOfBirth.trim().equals(""))||(husband!=null && !husband.trim().equals(""))){	//Changed two more entries
			search(target,dateOfBirth,husband);
	...
	private void search(String searchText,String dateOfBirth,String husband){
		searchIndexList.clear();
		searchedUserList.clear();
		List<User> users= new UserService().getUsers();	
		setTableValues(HMConstants.EDIT,users);
		dm.getRowCount();
			for(int row = 0; row < table.getRowCount(); row++){
				if(searchText!=null&&!searchText.equalsIgnoreCase("")){
					String next = (table.getValueAt(row, 1)!=null)?table.getValueAt(row, 1).toString() : "";
					if(matchSearchText(next,searchText)){
						if(table.getValueAt(row, 6) instanceof User){
							searchedUserList.add((User)table.getValueAt(row, 6));
						}
						searchIndexList.add(new TableIndex(row, 1));
					}
				}
				if(husband!=null&&!husband.equalsIgnoreCase("")){
					String next = (table.getValueAt(row, 3)!=null)?table.getValueAt(row,3).toString() : "";
					if(matchSearchText(next,husband)){
						if(table.getValueAt(row, 6) instanceof User){
							searchedUserList.add((User)table.getValueAt(row, 6));
						}
						searchIndexList.add(new TableIndex(row, 3));
					}
				}
				/*if(dateOfBirth!=null&&!dateOfBirth.equalsIgnoreCase("")){
					String next = (table.getValueAt(row, 2)!=null)?table.getValueAt(row,2).toString() : "";
					if(matchSearchText(next,dateOfBirth)){
						if(table.getValueAt(row, 6) instanceof User){
							searchedUserList.add((User)table.getValueAt(row, 6));
						}
						searchIndexList.add(new TableIndex(row, 2));
					}
				}*/
	...
	private void showSearchResults(int row, int col){
		r = table.getCellRect(row, col, true);//BugArea
